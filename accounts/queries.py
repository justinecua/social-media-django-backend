...
def login(username, password):
    ...
    try:
        ...
        query = """
                SELECT firstname, lastname, gender, bio, age, birthday, profile_photo, cover_photo, cover_photo_id_imagekit, profile_photo_id_imagekit, username, password, date_joined, glow.auth_user.id
                FROM glow.accounts_account
                INNER JOIN glow.auth_user
                ON glow.accounts_account.auth_user_id = glow.auth_user.id
...
def register(username, email, password):
    ...
    try:
        query_check_username = "SELECT id FROM glow.auth_user WHERE username = %s"
        cursor.execute(query_check_username, (username,))
        if cursor.fetchone():
            return {"message": "Username is already taken.", "status": 400}

        query_check_email = "SELECT id FROM glow.auth_user WHERE email = %s"
        ...
        query_insert_user = """
            INSERT INTO glow.auth_user (username, email, password, date_joined, is_superuser, first_name, last_name, is_staff, is_active)
        ...
        query_insert_account = """
            INSERT INTO glow.accounts_account (auth_user_id)
            VALUES (%s)
        """
...
def forgot_password(email):
    ...
    try:
        query_check_email = "SELECT id, username FROM glow.auth_user WHERE email = %s"
        cursor.execute(query_check_email, (email,))
        ...
        query_insert_token = """
            INSERT INTO glow.password_reset (user_id, token, expires_at)
            VALUES (%s, %s, %s)
            ON CONFLICT (user_id) 
